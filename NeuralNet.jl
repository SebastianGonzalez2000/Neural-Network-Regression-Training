...
function NeuralNet_nParams(d, nHidden)
    ...
    # Connections between hidden layers (+1 for biases)
    for h in 2:length(nHidden)
        nParams += (nHidden[h-1] + 1) * nHidden[h]
    end

    # Connections from last hidden layer to output (+1 for bias)
    nParams += nHidden[end] + 1
...
function NeuralNet_backprop(bigW, x, y, nHidden)
    ...
    if nLayers > 1
        # Last Layer of Hidden Weights, v[1:end-1] because v[end] is bias,
        # which is unaffected by previous layers
        backprop = err * (dh(z[end]) .* v[1:end-1])
        Gm[end] = backprop * [h(z[end-1]); 1]'

        # Other Hidden Layers, ignore last column of Wm[layer+1]
        # because it represents biases
